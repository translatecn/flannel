...
import (
	"fmt"
	"github.com/flannel-io/flannel/pkg/over/subnet"
	"strings"
	"sync"
...
func (bm *manager) GetBackend(backendType string) (Backend, error) { // vxlan
	bm.mux.Lock()
	defer bm.mux.Unlock()
...
func NewManager(ctx context.Context, sm *subnet.KubeSubnetManager, extIface *ExternalInterface) Manager {
	return &manager{
		ctx:      ctx,
		sm:       sm,
		extIface: extIface,
