...
import (
	...
	"github.com/flannel-io/flannel/pkg/over/retry"
...
type network struct {
	...
	dev       *vxlanDevice
	v6Dev     *vxlanDevice
	subnetMgr *subnet2.KubeSubnetManager
	mtu       int
}
...
func newNetwork(subnetMgr *subnet2.KubeSubnetManager, extIface *backend.ExternalInterface, dev *vxlanDevice, v6Dev *vxlanDevice, _ ip.IP4Net, lease *lease.Lease, mtu int) (*network, error) {
	nw := &network{
...
func (nw *network) Run(ctx context.Context) {
	...
	wg.Add(1)
	go func() {
		subnet2.WatchLeases(ctx, nw.subnetMgr, nw.SubnetLease, events)
...
func (nw *network) handleSubnetEvents(batch []lease.Event) {
	for _, event := range batch {
		...
		if event.Lease.EnableIPv4 && nw.dev != nil {
			...
			// directRouting是指远程主机在同一子网上，因此不需要vxlan。
			directRoute = netlink.Route{
				Dst: sn.ToIPNet(),
				Gw:  attrs.PublicIP.ToIP(),
			}
			if nw.dev.directRouting { // configmap 配置的，默认false
