...
func (ksm *KubeSubnetManager) GetNetworkConfig(ctx context.Context) (*Config, error) {
...
func (m *KubeSubnetManager) HandleSubnetFile(path string, config *Config, ipMasq bool, sn ip.IP4Net, ipv6sn ip.IP6Net, mtu int) error {
	...
	return WriteSubnetFile(path, config, ipMasq, sn, ipv6sn, mtu) //   /run/flannelsubnet.env
}
...
func newKubeSubnetManager(ctx context.Context, c clientset.Interface, sc *Config, nodeName, prefix string) (*KubeSubnetManager, error) {
	var err error
...
func NewSubnetManager(ctx context.Context, apiUrl, kubeconfig, prefix, netConfPath string, setNodeNetworkUnavailable bool) (*KubeSubnetManager, error) {
	var cfg *rest.Config
	var err error
	...
	sc, err := ParseConfig(string(netConf))
	if err != nil {
